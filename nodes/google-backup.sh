...
if [[ ${COMMAND} == 'backup-end' ]]; then
    ...
    rclone --config /root/.config/rclone/rclone.conf --drive-chunk-size=32M sync $TARGET GoogleDrive:/Backups/Proxmox/$HOSTNAME --exclude '*.log' -v --stats=60s --transfers=16 --checkers=16
